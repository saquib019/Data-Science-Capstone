...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        # Filter data for all launch sites
...
# Callback function for TASK 3 and TASK 4
@app.callback(
...
def update_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title='Payload vs. Launch Outcome for All Launch Sites')
    else:
        # Filter data for the selected launch site
        filtered_df = spacex_df[
            (spacex_df['Launch Site'] == selected_site) & (spacex_df['Payload Mass (kg)'] >= payload_range[0]) &
